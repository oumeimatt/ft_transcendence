<template>
	<div>
		<div class="Container h-screen bg-myblue col-span-2 
		md:col-span-2 lg:col-span-1  scrollbar scrollbar-track-zinc-900 
		scrollbar-thumb-zinc-600 max-h-screen  rounded-lg ">
			<div class="pb-4">
				...
				<button @click.prevent="showRooms" class="mt-8 flex 
					...
					<svg xmlns="http://www.w3.org/2000/svg" 
						...
						01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2
						 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 
						 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2
						  2 0 01-2-2v-2z" />
					</svg>
					<p  class="text-slate-400 lg:text-xl md:text-xl text-sm  
						font-bold hover:underline"> All channels 
					</p>
				</button>
					<div class="h-5/6 scrollbar mt-4 scrollbar-track-zinc-900
						scrollbar-thumb-zinc-600 max-h-2/5 ">
						<div v-if="showAllRooms" >
							<div v-for="ChatRoom in store.state.allRooms" :key="ChatRoom" >
								<div v-if="ChatRoom.ischannel == true"   class="flex justify-start 
									items-center space-x-2 mt-4"> 
									<svg v-if="ChatRoom.ispublic == false" 
										xmlns="http://www.w3.org/2000/svg" class=" lg:ml-8 h-8 w-8 fill-slate-300"
										fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
											<path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 
											4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 
											002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
									</svg>
									<img   v-if="ChatRoom.ispublic == true" 
										src="../assets/Public.png" class="lg:ml-7 h-8 w-10 
										fill-slate-300" fill="none" viewBox="0 0 24 24">
									<router-link @click.prevent="getMessagesMembers(ChatRoom.id)" 
										:to="{name:'ChatRoom', params: {name: ChatRoom.name, id: ChatRoom.id , isPublic: ChatRoom.ispublic}}" 
										class="font-bold text-slate-400 hover:underline cursor-pointer pl-1 "> 
										{{ ChatRoom.name }} 
									</router-link>
				...
				<div class=" mt-8  pb-8  ">
					...
					<div class="h-5/6 scrollbar scrollbar-track-zinc-900
					 	scrollbar-thumb-zinc-600 max-h-2/3 ">
						<div v-for="Room in store.state.rooms" :key="Room" >
							<div v-if="Room.ischannel == true"  
								class="flex justify-start items-center space-x-2 mt-4"> 
								<svg  v-if="Room.ispublic == false" xmlns="http://www.w3.org/2000/svg"
									class=" lg:ml-8 h-8 w-8 fill-slate-300" fill="none" viewBox="0 0 24 24"
									stroke="currentColor" stroke-width="1">
										<path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 
										2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
								</svg>
								<img  v-if="Room.ispublic == true" 
									src="../assets/Public.png" class="lg:ml-7 h-8 w-10 fill-slate-300" 
									fill="none" viewBox="0 0 24 24">
								<router-link @click.prevent="getMessagesMembers(Room.id)" 
									:to="{name:'ChatRoom', params: {name: Room.name , id: Room.id, isPublic: Room.ispublic}}" 
									class="font-bold text-slate-400 hover:underline cursor-pointer pl-1 "> {{ Room.name }} 
								</router-link>
							</div>
						</div>
					</div>
						...
						<div v-for="friend in store.state.friends" :key="friend">
							<div @click.prevent="createAndGetDm(friend.id)" 
								...
								<img  :src="store.methods.playerAvatar(friend)" 
									...
									<span class="font-semibold text-slate-400 hover:underline cursor-pointer mt-4 pb-4 ">
										<router-link :to="{name:'Chat', params: {name: friend.username }}" > 
										 	{{ friend.username }} 
										</router-link>
									</span> 
							</div>
				...
				<div v-if="showCreate" class="fixed inset-36 z-50 ">
					<div class=" my-6 mx-auto max-w-lg  ">
						...
						<div class="border-0 rounded-lg shadow-lg w-full bg-neutral-100  ">
							...
							<form class=" grid gap-3 grid-cols-1  p-6 border-t 
								...
								<div v-if="room.players.length > 0" 
									class="flex scrollbar scrollbar-track-slate-300 scrollbar-thumb-slate-600">
									<div v-for="player in room.players" :key="player" 
										class="bg-slate-300 text-slate-700 w-fit h-fit p-2 mb-2 rounded mr-2"> 
											{{ player }}
									 </div>
								</div>
							</form>
      		...
      		<div class=" my-6 mx-auto max-w-sm text-center ">
        		<div class="border-0 rounded-lg shadow-lg w-full bg-white  ">
        			<div class=" p-5 border-b border-solid border-slate-200 rounded-t">
          			...
          			<div class="flex items-center justify-center space-x-8 
						p-6 border-t border-solid border-slate-200 rounded-b">
            			<router-link @click.prevent="store.state.connection.emit('invitation-accepted', opponent);" 
            			...
            			<button @click.prevent="invited = false ; opponent = ''" 
							class="text-gray-800 font-bold hover:border hover:rounded 
							hover:border-solid hover:white hover:text-white
							 hover:bg-slate-800 uppercase px-6 py-3 text-sm outline-none    ">
              				Refuse
            			</button>
         			</div>
        		</div>
