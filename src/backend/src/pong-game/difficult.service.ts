...
@Injectable()
  ...
  readonly emptyPlayground = new PlayGround(0, 0, 1000, 600, '#0A63A3ff', 9, true, '', '');
  ...
  joinPlayersToGame(first: Socket, second: Socket, wss: Server) {
    const roomname = first.id + '+' + second.id;
        ...
        const playground = new PlayGround(0, 0, 1000, 600, '#0A63A3ff', 9, true, first.data.user.username, second.data.user.username);
  ...
  async gameFinished(first: Socket, second: Socket, playground: PlayGround, wss: Server): Promise<void> {
    ...
    if (playground.scoreBoard.playerOneScore > playground.scoreBoard.playerTwoScore) {
      try {
        await this.usersService.updateLevel(first.data.user.id, true);
      } catch (err) {
      ...
      try {
        await this.usersService.updateLevel(second.data.user.id, true);
      } catch (err) {
        this.logger.error('Couldn\'t Update Level');
      }
      try {
      ...
      try {
        ...
        // send event with winner and loser
        wss.to(first.data.roomname).emit('DisplayWinner', { winner: second.data.user.username, loser: second.data.user.username });
      } catch (err) {
        this.logger.error('Couldn\'t Save Game');
      }
    }
