...
let OneVOneService = class OneVOneService {
    ...
    async handlePlayerConnected(client, players, wss) {
        ...
        if (found && found.status === player_status_enum_1.UserStatus.PLAYING) {
            ...
            if (!first) {
                ...
                const second = client;
                try {
                    await this.usersService.updateStatus(second.data.user.id, player_status_enum_1.UserStatus.PLAYING);
                }
                catch (err) {
                    this.logger.error('Couldn\'t Update Status');
                    return;
    ...
    joinPlayersToGame(first, second, wss) {
        ...
        const playground = new utils_1.PlayGround(0, 0, 1000, 600, '#0A5B70ff', 13, false, first.data.user.username, second.data.user.username);
